...
bot = Bot(token=bot_settings.BOT_TOKEN, parse_mode=types.ParseMode.MARKDOWN)
...
async def send_reply(message: types.Message, state: FSMContext):
    ...
    if message.from_user.id == expected_admin_id:
        try:
            await bot.send_message(chat_id=user_id, text=f'❗️Администратор отправил вам сообщение: {message.text}', reply_markup=inline_markup)
